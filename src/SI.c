...
unsigned char adcBuffer[3];    // ADC와 통신하기 위해 사용되는 버퍼
...
// ADC 채널에 따른 주소를 adcBuffer에 설정
void setConfigBit(int channel)
{
    adcBuffer[0] = 0b110; // 채널에 따른 시작 바이트 설정
    adcBuffer[1] = channel<<6;
    adcBuffer[2] = 0;
}
...
int readDustSensor(){
    ...
    wiringPiSPIDataRW(0, adcBuffer, 3); // SPI 장치와 데이터 교환
    digitalWrite(CS, HIGH); // CS/SHDN bit : CHANNEL 바뀔 때 1로 setting
    
    delayMicroseconds(DELTA);
    digitalWrite(LED,HIGH);
    adc = ((adcBuffer[1] & 0x0F) << 8) + adcBuffer[2]; // 데이터 교환 후 ADC 값 버퍼 추출, 반환
    return adc;
}
